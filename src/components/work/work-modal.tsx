  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 md:p-4 max-sm:p-2 max-sm:pb-[max(env(safe-area-inset-bottom),0.5rem)]">
      ...
      >
        ...
        <button
          onClick={onClose}
          className="absolute top-4 right-4 md:top-6 md:right-5 rounded-full transition-colors z-10 cursor-pointer size-10 flex items-center justify-center mobile-tap-target"
          aria-label="Close modal"
        >
          <X className="size-4 md:size-6 text-foreground" />
        </button>
        ...
        <section className="w-full">
          ...
          <div className="flex flex-wrap gap-2">
            {narrative.stackHighlights.map((tech) => (
              <EvidenceChip key={tech} value={tech} />
            ))}
          </div>
        </section>
      </m.div>
